...
/**
 * Interface para opções de selects
 */
...
export interface RDM {
  ...
  status?: string; // Status atual (ex: pendente, aprovado, reprovado, corrigir)
  ...
  approvedDate?: string; // Data de aprovação (verificar se é necessário)
  approvedBy?: string; // Quem aprovou (verificar se é necessário)
...
/**
 * Modelo para aprovação/Reprovação de uma RDM
...
export interface RDMList extends RDM {
  ...
  rejectedDate?: string; // Data de Reprovação (se aplicável)
  rejectedBy?: string;
  rejectedReason?: string; // Rejeitado por (se aplicável)
