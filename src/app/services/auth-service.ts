...
export class AuthService {
  ...
  private decodeToken(token: string): UserProfile | null {
    try {
      ...
      return {
        id: decoded.sid || decoded.sub || 0, // sid é comum em tokens .NET
  ...
  isAdmin(): boolean {
    ...
    return roleLower === 'administrador' || roleLower === 'admin' || roleLower.includes('admin');
  }
  ...
  isTeamMember(): boolean {
    ...
    const roleLower = user.role.toLowerCase();
    return (
      roleLower === 'teammember' || roleLower === 'membro do time' || roleLower.includes('team')
    );
  }
