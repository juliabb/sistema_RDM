...
export class RdmService {
  ...
  // Endpoint para cancelar um RDM (disponível para usuários comuns)
  cancelRDM(ticket: string): Observable<any> {
    const url = buildApiUrl(API_PATHS.RDM_CANCEL(ticket));
    return this.http.put(url, null); // endpoint não espera body
  }
  ...
  getAllRDM(params: RDMSearchParams = {}): Observable<RDMPagedResult> {
    ...
    // Se houver filtros, precisamos de TODOS os dados para filtrar corretamente.
    if (hasFilters) {
      // Busca com pageSize grande o suficiente para trazer tudo.
      const largeParams = new HttpParams().set('PageNumber', '1').set('PageSize', '1000');

      const url = buildApiUrl(API_PATHS.ADMIN_GET_ALL_USERS);
      return this.http.get<any[]>(url, { headers, params: largeParams, observe: 'response' }).pipe(
        map((response) => {
          const body = response.body || [];
          const allRDM = this.transformAllUsersResponse(body);
          const filtered = this.applyRDMFilters(allRDM, params);
          const sorted = this.sortRDM(filtered, params.sortBy, params.sortOrder);
          return this.createPagedResult(sorted, params.page || 1, params.pageSize || 10);
        }),
        catchError(() => of(this.createEmptyPagedResult(params))),
      );
    }
    ...
    return this.http
      .get<any[]>(url, {
        headers,
        params: httpParams,
        observe: 'response', // cabeçalhos
      })
      .pipe(
        map((response) => {
          const body = response.body || [];
          const items = this.transformAllUsersResponse(body);

          // Extrai o cabeçalho de paginação
          const paginationHeader = response.headers.get('Pagination');
          let serverTotal = items.length; // fallback
          if (paginationHeader) {
            try {
              const pagination = JSON.parse(paginationHeader);
              serverTotal = pagination.totalItems || items.length;
            } catch {
          ...
          return {
            ...
            totalPages: Math.ceil(serverTotal / (params.pageSize || 10)),
          };
        }),
        catchError(() => of(this.createEmptyPagedResult(params))),
      );
  ...
  /**
   * Busca os dados de validação pós‑implementação de um ticket específico.
   * Utiliza o endpoint de listagem e filtra pelo ticket.
   */
  getValidacaoPorTicket(ticketId: string): Observable<any> {
    const url = buildApiUrl(API_PATHS.ADMIN_RDM_VALIDATION);
    const headers = this.getHeaders();
    // Define um pageSize grande para obter todas as validações (ajuste conforme necessidade)
    const params = new HttpParams().set('PageNumber', '1').set('PageSize', '100'); // Aumente se houver mais de 100 validações

    return this.http.get<any[]>(url, { headers, params }).pipe(
      map((validacoes) => validacoes.find((v) => v.ticket === ticketId) || null),
      catchError((error) => {
        console.error('Erro ao carregar validações pós‑implementação', error);
        return of(null); // Retorna null para que a UI exiba "Aguardando validação"
      }),
    );
  }

  /**
   * Atualiza status de um RDM (aprovação/reprovação)
   * @param ticketId ID do ticket do RDM
  ...
  /**
   * Retorna a URL correta para anexos baseada no papel do usuário
   */
  private getAttachmentUrl(ticketId: string): string {
    if (this.authService.isAdmin()) {
      return buildApiUrl(API_PATHS.ADMIN_ATTACHMENTS(ticketId));
    } else {
      return buildApiUrl(API_PATHS.RDM_ATTACHMENTS(ticketId));
    }
  }
  ...
  downloadAttachment(ticketId: string): Observable<Blob> {
    const headers = this.getHeaders();
    const url = this.getAttachmentUrl(ticketId);
  ...
  checkAttachmentExists(ticketId: string): Observable<boolean> {
    ...
    return this.http.get(url, { headers, observe: 'response', responseType: 'blob' }).pipe(
      ...
      catchError((error) => {
        ...
        if (error.status === 404) {
          return of(false); // Não tem anexo
        }
        // Para outros erros, também consideramos como falso (ou pode relançar)
        return of(false);
      }),
    );
  }
  ...
  private transformAllUsersResponse(apiData: any[]): RDMList[] {
    ...
    return apiData.map((item) => {
      ...
      return {
        ...
        status: status,
        department: item.department || 'Não especificado',
        ...
        // 👇 mapeia campo da API para o campo esperado
        rejectedReason: item.rejectedReason || item.rejectionReason || '',
